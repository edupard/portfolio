...
class NetShivaV1(object):
    ...
    def eval(self, state, input, labels, mask, seq_len):
        feed_dict = {self.input: input, self.labels: labels, self.mask: mask, self.keep_prob: 1.0, self.seq_len: seq_len}
    ...
    def fit(self, state, input, labels, mask, seq_len):
        feed_dict = {self.input: input, self.labels: labels, self.mask: mask, self.keep_prob: self.config.DROPOUT_KEEP_RATE, self.seq_len: seq_len}
        self._fill_feed_dict(feed_dict, state)
