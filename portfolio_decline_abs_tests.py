                ...
                for i in range(total_days):
                    ...
                    if trading_day:
                        ...
                        if open_pos:
                            ...
                            # Abs variant
                            metric = DECLINE_LONG_FACTOR * -decline + (1 - DECLINE_LONG_FACTOR) * prediction

                            metric_sorted = np.sort(np.abs(metric[pos_mask]))
                            bound_idx = max(-SELECTION, -metric_sorted.shape[0])
                            bound = metric_sorted[bound_idx]
                            prediction_pos_mask = metric >= bound
                            prediction_pos_mask |= metric <= -bound
                            ...
                            # # Factors do not play vs each other
                        ...
                        prev_px = curr_px
                        prev_pos_mask = tradeable & in_snp
