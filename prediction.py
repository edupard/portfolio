...
ONE_DAY_DATE = datetime.datetime.strptime('2017-12-01', '%Y-%m-%d').date()
...
for index, row in pred_ds.iterrows():
    ...
    prediction_date_idx, _ = ds.get_data_range(PREDICTION_DATE, PREDICTION_DATE)
    prediction_px = ds.get_a_c(one_week_date_idx)
    ...
    hpr_px = ds.get_a_c(open_pos_date_idx)

    one_day_v = ds.get_a_v(one_day_date_idx)
    one_week_v = np.mean(ds.get_a_v(one_day_date_idx, prediction_date_idx + 1))
...
result_df = pd.DataFrame({
    ...
    '*': [PREDICTION_DATE.strftime('%Y-%m-%d')] * final_tickers_count,
    ...
    '# px': open_pos_px_arr,
    'hp px': hpr_px_arr,
    '1w pct': one_week_pct_arr,
    '1d pct': one_day_pct_arr,
    'hpr pct': hp_pct_arr,
    '1d v': one_day_v_arr,
